...
//				$data['barcode'] = $product['sku'];
        ...
        $results = $this->model_export_yandex_market->getProductImages($product['product_id']);
//          foreach($this->model_catalog_product->getProductImages($product['product_id']) as $result){
//            $data['pictures'][] = $this->model_tool_image->resize($result['image'], 100, 100);
//          }
	...
	private function setOffer($data) {
		...
		$allowed_tags = array('url'=>0, 'buyurl'=>0, 'price'=>1, 'wprice'=>0, 'currencyId'=>1, 'xCategory'=>0, 'categoryId'=>1, 'picture'=>0, 'pictures'=>0, 'store'=>0, 'pickup'=>0, 'delivery'=>0, 'deliveryIncluded'=>0, 'local_delivery_cost'=>0, 'orderingTime'=>0);
