...
class ControllerCatalogCategory extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['image_big'])) {
			...
			$data['image_big'] = $category_info['image_big'];
		} else {
			$data['image_big'] = '';
		}
		...
		} elseif (!empty($category_info) && is_file(DIR_IMAGE . $category_info['image_big'])) {
			$data['thumb_big'] = $this->model_tool_image->resize($category_info['image_big'], 100, 100);
		} else {
		...
		if (isset($this->request->post['left'])) {
			...
			$data['left'] = $category_info['left'];
		} else {
			$data['left'] = 0;
		}

		if (isset($this->request->post['left_cat'])) {
			$data['left_cat'] = $this->request->post['left_cat'];
		} elseif (!empty($category_info)) {
			$data['left_cat'] = $category_info['left_cat'];
		} else {
			$data['left_cat'] = 0;
		}
